...
class ReceiveSyncProperty : AbstractProperty() {
...
//		MainUI.getDocument().fire(Repository.ChangeType.UPDATED)
	...
	private fun getChannels (): List<String> {
		val nsta = MainUI.load(MainUI.getDocument().get())
	...
	private fun getChannelTemplates(channel: String): Set<String> {
		val nsta = MainUI.load(MainUI.getDocument().get())
		return nsta.template
				.flatMap { it.edge }
				.filter { it.synchronization != null && it.synchronization.kind == SynchronizationKind.RECEIVE }
				.filter { it.synchronization.channelExpression.identifier.name == channel }
				.map { it.parentTemplate.name }
				.toSet()
	}
